...
Assembles a test program that exercises every opcode, runs it on both
the Python VM and the Rust VM, and compares register state.

Usage:
    python3 tools/conformance_test.py
"""
...
# ═══════════════════════════════════════════════════════════════
# FLUX Bytecode assembler (minimal — enough for conformance testing)
# ═══════════════════════════════════════════════════════════════

# Opcode constants (from FLUX_BYTECODE_SPEC.md)
HALT = 0x00
NOP  = 0x06
MOV  = 0x01  # C: rd, rs
...
JE   = 0x2E  # D: r, off16
JNE  = 0x2F  # D: r, off16
FADD = 0x40  # E: rd, rs1, rs2
FSUB = 0x41  # E: rd, rs1, rs2
FMUL = 0x42  # E: rd, rs1, rs2
FDIV = 0x43  # E: rd, rs1, rs2
PRINT = 0x30 # B: r
...
def run_python_vm(bytecode_path):
    """Run bytecode on the Python FLUX VM and return register state."""
    try:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        # Try different import paths
        try:
            from flux.vm import VM
        except ImportError:
            try:
                from fluxvm import VM
            except ImportError:
                # Use the installed package
                from flux_vm import VM
        ...
        # Extract registers
        regs = {}
        if hasattr(vm, 'registers'):
            for i in range(16):
                val = vm.registers[i] if hasattr(vm.registers, '__getitem__') else getattr(vm.registers, f'r{i}', None)
                if val is not None:
                    # Convert to signed 32-bit
                    if isinstance(val, int):
                        regs[i] = val if val < 0x80000000 else val - 0x100000000
                    else:
                        regs[i] = val
        elif hasattr(vm, 'regs'):
            regs = vm.regs
        return regs
